...
class SetNewPassword extends Component {
    ...
    if (validate(schema).isValid) {
      ...
      awsLoginRequest(userCred).then((res) => {
        if (res.error === 'New password required') {
          res.cognitoUser.completeNewPasswordChallenge(values.newPassword, attributesData, {
  ...
  render() {
    ...
    return (
      <form onSubmit={(event) => this.handleSubmitFrom(event)} style={{ minHeight: 650 }} className="d-flex justify-content-center align-items-center">
        <Container fluid>
